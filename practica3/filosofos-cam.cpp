...
void funcion_camarero(){
	...
	while (true){
		...
		if (num_filosofos_mesa < (num_filosofos - 1 )){
		...
		{
         case etiq_sentarse: // si ha sido el productor: insertar en buffer
			num_filosofos_mesa++;
            MPI_Ssend( &valor, 1, MPI_INT, estado.MPI_SOURCE , etiq_sentarse, MPI_COMM_WORLD);
            break;

         case etiq_levantarse: // si ha sido el consumidor: extraer y enviarle
            num_filosofos_mesa--;
            break;
		}
...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
	cout <<"Filósofo " <<id << " solicita sentarse  al camarero." <<endl;
    ...
    cout <<"Camarero sento al filosofo " <<id << " en la mesa." <<endl;
    ...
    sleep_for( milliseconds( aleatorio<1000,10000>() ) );
    ...
    cout <<"Filósofo " <<id << " solicita levantarse al camarero." <<endl;
    MPI_Ssend( &valor, 1, MPI_INT, id_camarero, etiq_levantarse, MPI_COMM_WORLD);
    cout <<"Camarero levanto al filosofo " <<id << " de la mesa." <<endl;
