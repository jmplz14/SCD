...
int producir(int num_orden)
{
   ...
   int valor_producido = num_orden * (num_items/num_productores) + contador;
   
   
   sleep_for( milliseconds( aleatorio<10,100>()) );
   contador++ ;
   cout << "Productor ha producido valor " << valor_producido << endl << flush;
...
{
   int num_pasadas = (num_items / num_productores);
   for ( unsigned int i=0 ; i < num_pasadas ; i++ )
   {
      // producir valor
...
void funcion_consumidor(int num_orden)
{
   int         peticion,
               valor_rec = 1,
               num_pasadas = (num_items / num_consumidores);
               //num_proceso;
               
   MPI_Status  estado ;
   
   //MPI_Comm_rank(MPI_COMM_WORLD, &num_proceso);            
   for( unsigned int i=0 ; i < num_pasadas ; i++ )
   {
